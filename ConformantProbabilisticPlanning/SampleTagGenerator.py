...
class SampleTagsGenerator:
    def __init__(self, problem, candidate_plan, action_map, contexts, tag_generator, all_projected_problems):
        ...
        self.number_of_satisfied_tags = 0
        self.used_projected_problems_for_sample_tags = set()
        self.used_projected_problems_for_satisfied_tags = set()
    ...
    def find_random_sample_tags(self, threshold=None):
        ...
        for tag in all_tags_in_one_list:
            ...
            # context_assert = constraint_object.get_projected_context_assert()
            ...
            if solver.check() != sat:
                ...
                if satisfied_tags[pro_index] is None:
                    satisfied_tags[pro_index] = [tag]
                else:
                    satisfied_tags[pro_index].append(tag)
                self.number_of_satisfied_tags += 1
                self.used_projected_problems_for_satisfied_tags.add(pro_index)
        print('没有足够多的sample tag')
        return None, final_probability
        # mp_satisfied = MergedProblems(self.all_projected_problems, satisfied_tags, self.number_of_satisfied_tags,
        #                     self.used_projected_problems_for_satisfied_tags, self.problem)
        # mp_satisfied.merge_problems()
        # tp = TagProbability(self.problem, self.candidate_plan, self.action_map, self.contexts, self.tag_generator, mp_satisfied)
        # satisfied_states_probability = self.compute_satisified_tags_probability(satisfied_tags, tp)
        # if satisfied_states_probability >= threshold:
        #     print('没有足够多的sample tag')
        #     return None, final_probability
        # else:
        #     print('发现无法执行的tag，当前counter-tag概率不满足阈值要求')
        #     print('satisfied_states_probability', satisfied_states_probability)
        #     # print(satisfied_tags)
        #     if not has_sample_tags:
        #         print('没有多余sample tag, 此题无解')
        #         return None, None
        #     else:
        #         print('使用数量不足的sample tags')
        #         self.sample_tags = sample_tags
    ...
    def get_information_from_sample_tags_for_hitting_set(self, sample_tags_history, tag_index_map):
        res_num = 0
        sample_tag_index = set()

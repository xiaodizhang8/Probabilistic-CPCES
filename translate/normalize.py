...
from translate.pddl.conditions import Falsity
...
class AxiomConditionProxy(ConditionProxy):
    ...
    def build_rules(self, rules):
        ...
        eff_rule_head = Atom(axiom.name, [par.name for par in params])
...
def get_action_predicate(action):
    ...
    if isinstance(action.precondition, ExistentialCondition):
...
def remove_universal_quantifiers(task):
    def recurse(condition):
        ...
        if isinstance(condition, UniversalCondition):
            ...
            typed_parameters = tuple(TypedObject(v, type_map[v]) for v in parameters)
            ...
            return NegatedAtom(axiom.name, parameters)
        else:
...
def split_disjunctions(task):
    for proxy in tuple(all_conditions(task)):
        ...
        if isinstance(proxy.condition, Disjunction):
...
def eliminate_existential_quantifiers_from_axioms(task):
    ...
    for axiom in task.axioms:
        ...
        if isinstance(precond, ExistentialCondition):
...
def condition_to_rule_body(parameters, condition):
    ...
    if not isinstance(condition, Truth):
        ...
        for part in parts:
            ...
            assert isinstance(part, Literal), "Condition not normalized: %r" % part
            if not part.negated:
                result.append(part)
    return result

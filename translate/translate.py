...
from translate.pddl.conditions import Literal
...
def translate_strips_conditions_aux(conditions, dictionary, ranges):
    ...
    for fact in conditions:
        if fact.negated:
            ...
            new_condition = {}
            atom = Atom(fact.predicate, fact.args)  # force positive
...
def translate_strips_operator(operator, dictionary, ranges, mutex_dict,
                              mutex_ranges, implied_facts, options):
    ...
    for condition in conditions:
        op = translate_strips_operator_aux(operator, dictionary, ranges,
                                           mutex_dict, mutex_ranges,
                                           implied_facts, condition, options)
        if op is not None:
...
def translate_strips_operator_aux(operator, dictionary, ranges, mutex_dict,
                                  mutex_ranges, implied_facts, condition, options):
    ...
    for conditions, fact in operator.del_effects:
        ...
        for var, val in dictionary[new_fact]:
    ...
    return build_sas_operator(operator.name, condition, effects_by_variable,
                              operator.cost, ranges, implied_facts, options)


def build_sas_operator(name, condition, effects_by_variable, cost, ranges,
                       implied_facts, options):
    if options['add-implied-preconditions']:
    ...
    for var, effects_on_var in effects_by_variable.items():
        ...
        for post, eff_conditions in effects_on_var.items():
            ...
            if ranges[var] == 2:
                ...
                if (options['add-implied-preconditions']and pre == -1 and
...
def translate_task(strips_to_sas, ranges, translation_key,
                   ...
                   init, goals,
                   actions, axioms, metric, implied_facts, options):
    with timers.timing("Processing axioms", block=True):
        axioms, axiom_layer_dict = axiom_rules.handle_axioms(actions, axioms, goals,
                                                             options['layer-strategy'])

    if options["dump-task"]:
    ...
    if not goal_pairs:
        return 'Simplified to empty goal'
...
def pddl_to_sas(task, options):
    ...
    if not relaxed_reachable:
        return unsolvable_sas_task("No relaxed solution"), actions

    # HACK! Goals should be treated differently.
    if isinstance(task.goal, Conjunction):
    ...
    with timers.timing("Computing fact groups", block=True):
        groups, mutex_groups, translation_key = fact_groups.compute_groups(
            task, atoms, reachable_action_params, options)
    ...
    with timers.timing("Building mutex information", block=True):
        if options['use_partial_encoding']:
            mutex_key = build_mutex_key(strips_to_sas, mutex_groups, options)
        else:
    ...
    if sas_task == 'Simplified to empty goal':
        return 'Simplified to empty goal', actions
    ...
    if options['filter_unreachable_facts']:
        with timers.timing("Detecting unreachable propositions", block=True):
            try:
                ...
                return unsolvable_sas_task("Simplified to trivially false goal"), actions
...
def build_mutex_key(strips_to_sas, groups, options):

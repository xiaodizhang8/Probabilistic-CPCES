...
class Effect:
    ...
    def instantiate(self, var_mapping, init_facts, fluent_facts,
        ...
        if self.parameters:
            ...
            self._instantiate(var_mapping, init_facts, fluent_facts, result)
    def instantiate_with_delete_effects(self, var_mapping, init_facts, fluent_facts, objects_by_type, result):
        if self.parameters:
            var_mapping = var_mapping.copy()  # Will modify this.
            object_lists = [objects_by_type.get(par.type_name, [])
                            for par in self.parameters]
            for object_tuple in cartesian_product(*object_lists):
                for (par, obj) in zip(self.parameters, object_tuple):
                    var_mapping[par.name] = obj
                self._instantiate_with_delete_effects(var_mapping, init_facts, fluent_facts, result)
        else:
            self._instantiate_with_delete_effects(var_mapping, init_facts, fluent_facts, result)
    ...
    def _instantiate_with_delete_effects(self, var_mapping, init_facts, fluent_facts, result):
        condition = []
        try:
            self.condition.instantiate_with_delete_effects(var_mapping, init_facts, fluent_facts, condition)
        except Impossible:
            return
        effects = []
        self.literal.instantiate_with_delete_effects(var_mapping, init_facts, fluent_facts, effects)
        assert len(effects) <= 1
        if effects:
            result.append((condition, effects[0]))
    def relaxed(self):
...
class ConditionalEffect:
    def __init__(self, condition, effect):
        if isinstance(effect, ConditionalEffect):
            self.condition = Conjunction([condition, effect.condition])

...
from translate.pddl.conditions import Impossible
...
class Action:
    def __init__(self, name, parameters, num_external_parameters,
        ...
        self.all_preconditions = precondition
    ...
    def instantiate(self, var_mapping, init_facts, init_assignments,
        ...
        all_preconditions = []
        try:
            self.all_preconditions.instantiate_with_delete_effects(var_mapping, init_facts,
                                                                   fluent_facts, all_preconditions)
        except Impossible:
            return None
        effects = []
        effects_with_delete = []
        for eff in self.effects:
            ...
            eff.instantiate_with_delete_effects(var_mapping, init_facts, fluent_facts,
                            objects_by_type, effects_with_delete)
        if effects:
            if metric:
                ...
                cost = 1
            return PropositionalAction(name, precondition, effects, cost, effects_with_delete, all_preconditions)
        else:
            return None


class PropositionalAction:
    def __init__(self, name, precondition, effects, cost, effects_with_delete, all_preconditions):
        self.name = name
        self.precondition = precondition
        self.add_effects = []
        self.del_effects = []
        self.all_del_effects = []
        self.all_add_effects = []
        self.effects_with_delete = effects_with_delete
        self.all_preconditions = all_preconditions
        for condition, effect in effects:
            if not effect.negated:
                self.add_effects.append((condition, effect))
        for condition, effect in effects_with_delete:
            if not effect.negated:
                self.all_add_effects.append((condition, effect))
        ...
        for condition, effect in effects:
            if effect.negated and (condition, effect.negate()) not in self.add_effects:
                self.del_effects.append((condition, effect))
        for condition, effect in effects_with_delete:
            if effect.negated and (condition, effect.negate()) not in self.add_effects:
                self.all_del_effects.append((condition, effect))
        self.cost = cost

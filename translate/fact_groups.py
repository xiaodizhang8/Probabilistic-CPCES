    ...
    for fact in group:
        try:
            ...
            for obj in task.objects:
                ...
                atom = Atom(fact.predicate, newargs)
...
def choose_groups(groups, reachable_facts, options):
    queue = GroupCoverQueue(groups)
    uncovered_facts = reachable_facts.copy()
    result = []
    while queue:
        group = queue.pop(options)
...
def compute_groups(task, atoms, reachable_action_params, options):
    groups = invariant_finder.get_groups(task, options, reachable_action_params)
    ...
    with timers.timing("Choosing groups", block=True):
        groups = choose_groups(groups, atoms, options)
    groups = sort_groups(groups)

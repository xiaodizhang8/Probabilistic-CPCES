...
from translate import constraints
...
def get_literals(condition):
    if isinstance(condition, Literal) or isinstance(condition, Atom) or isinstance(condition, NegatedAtom):
        yield condition
    elif isinstance(condition, Conjunction) or isinstance(condition, Disjunction):
        for item in condition.parts:
            if isinstance(item, Conjunction) or isinstance(condition, Disjunction):
